...
def main():
    ...
    with tf.name_scope("create_inputs"):
        reader = ImageReaderPGN(DATA_DIR, LIST_PATH, DATA_ID_LIST, INPUT_SIZE, RANDOM_SCALE, RANDOM_MIRROR, SHUFFLE,
                                coord)
    ...
    for i in xrange(num_gpus):
        with tf.device('/gpu:%d' % i):
            with tf.name_scope('Tower_%d' % (i)) as scope:
                ...
                edge_cond = tf.multiply(tf.cast(tf.greater(raw_edge, 0.1), tf.int32),
                                        tf.cast(tf.less_equal(raw_gt, N_CLASSES - 1), tf.int32))
                ...
                loss_p1_gb = tf.reduce_mean(
                    tf.nn.sparse_softmax_cross_entropy_with_logits(logits=prediction_p1, labels=gt))
                loss_p2_gb = tf.reduce_mean(
                    tf.nn.sparse_softmax_cross_entropy_with_logits(logits=prediction_p2, labels=gt))
                loss_p1_lc = tf.reduce_mean(
                    tf.nn.sparse_softmax_cross_entropy_with_logits(logits=p1_lc, labels=gt_edge))
                loss_p2_lc = tf.reduce_mean(
                    tf.nn.sparse_softmax_cross_entropy_with_logits(logits=p2_lc, labels=gt_edge))
                ...
                loss_e1_res5_pos_lc = tf.reduce_sum(
                    tf.multiply(tf.multiply(t_loss_e1_res5, parsing_mask), edge_pos_mask), [1, 2])
                loss_e1_res5_neg_lc = tf.reduce_sum(
                    tf.multiply(tf.multiply(t_loss_e1_res5, parsing_mask), edge_neg_mask), [1, 2])
                ...
                loss_e1_res4_pos_lc = tf.reduce_sum(
                    tf.multiply(tf.multiply(t_loss_e1_res4, parsing_mask), edge_pos_mask), [1, 2])
                loss_e1_res4_neg_lc = tf.reduce_sum(
                    tf.multiply(tf.multiply(t_loss_e1_res4, parsing_mask), edge_neg_mask), [1, 2])
                ...
                loss_e1_res3_pos_lc = tf.reduce_sum(
                    tf.multiply(tf.multiply(t_loss_e1_res3, parsing_mask), edge_pos_mask), [1, 2])
                loss_e1_res3_neg_lc = tf.reduce_sum(
                    tf.multiply(tf.multiply(t_loss_e1_res3, parsing_mask), edge_neg_mask), [1, 2])
    ...
    restore_var = [v for v in all_saver_var if
                   'parsing' not in v.name and 'edge' not in v.name and 'Momentum' not in v.name]
    ...
    for step in range(NUM_STEPS):
        ...
        # Apply gradients.
        summary, loss_value, par_loss, edge_loss, _ = sess.run(
        ...
        print('step {:d} \t loss = {:.3f}, parsing_loss = {:.3f}, edge_loss = {:.3f}, ({:.3f} sec/step)'.format(step,
                                                                                                                loss_value,
                                                                                                                par_loss,
                                                                                                                edge_loss,
                                                                                                                duration))
