...
def conv2d(input_, output, kernel, stride, relu, bn, name, stddev=0.01):
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        # Add the biases
        b = tf.compat.v1.get_variable('b', [output], initializer=tf.constant_initializer(0.0))
...
def max_pool(input_, kernel, stride, name):
    return tf.nn.max_pool(input_, ksize=[1, kernel, kernel, 1], strides=[1, stride, stride, 1], padding='SAME',
                          name=name)
...
def linear(input_, output, name, stddev=0.02, bias_start=0.0):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        bias = tf.compat.v1.get_variable("bias", [output], initializer=tf.constant_initializer(bias_start))
...
def gcn(input_, output, kernel, stride, relu, bn, name, stddev=0.01):
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        w1_1 = tf.compat.v1.get_variable('w1_1', left_shape_k_1,
                                         initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev))
        w1_2 = tf.compat.v1.get_variable('w1_2', left_shape_1_k,
                                         initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev))
        w2_1 = tf.compat.v1.get_variable('w2_1', right_shape_1_k,
                                         initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev))
        w2_2 = tf.compat.v1.get_variable('w2_2', right_shape_k_1,
                                         initializer=tf.compat.v1.truncated_normal_initializer(stddev=stddev))
        b1_1 = tf.compat.v1.get_variable('b1_1', [output], initializer=tf.constant_initializer(0.0))
        b1_2 = tf.compat.v1.get_variable('b1_2', [output], initializer=tf.constant_initializer(0.0))
        b2_1 = tf.compat.v1.get_variable('b2_1', [output], initializer=tf.constant_initializer(0.0))
        b2_2 = tf.compat.v1.get_variable('b2_2', [output], initializer=tf.constant_initializer(0.0))

        conv1_1 = tf.nn.conv2d(input_, w1_1, strides=[1, stride, stride, 1], padding='SAME')
        conv1_1 = tf.nn.bias_add(conv1_1, b1_1)
        if bn:
            conv1_1 = tf.compat.v1.layers.batch_normalization(conv1_1)
        if relu:
        ...
        if bn:
            conv1_2 = tf.compat.v1.layers.batch_normalization(conv1_2)
        if relu:
        ...
        if bn:
            conv2_1 = tf.compat.v1.layers.batch_normalization(conv2_1)
        if relu:
        ...
        if bn:
            conv2_2 = tf.compat.v1.layers.batch_normalization(conv2_2)
        if relu:
